  ...
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        // Section scrolled INTO view — fade in
        entry.target.classList.add('visible');
      } else {
        // Section scrolled OUT of view — reset so it fades in again
        entry.target.classList.remove('visible');
      }
    });
  }, { threshold: 0.1 });

  // No unobserve — keeps watching forever for both directions
  document.querySelectorAll('.scroll-reveal').forEach(el => {
    observer.observe(el);
  });
}

// Wait for boot screen to fully finish before starting scroll reveal
const mainContent = document.getElementById('main-content');

const bootWatcher = new MutationObserver(() => {
...
bootWatcher.observe(mainContent, {
  attributes: true,
